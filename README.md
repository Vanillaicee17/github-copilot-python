...
- **Check** — compares filled, non-given cells against the actual stored solution (kept server-side in the Flask session, never sent to the client) and highlights any that are wrong; empty non-given cells are highlighted too, since an unfilled cell isn't done. The congratulatory message only appears once every cell is filled and none are flagged wrong.
